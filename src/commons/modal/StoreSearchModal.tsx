...
import { KAKAO_REST_TOKEN, KAKAO_MAP_TOKEN } from '@constants/api';
import '@styles/commons/modal/StoreSearchModal.scss';
...
interface StoreSearchModalProps {
  onClose: () => void;
  setStoreAddress: (address: string) => void; // 주소 문자열을 인자로 받아 처리하는 함수라고 가정했습니다.
}

const StoreSearchModal: React.FC<StoreSearchModalProps> = ({
  onClose,
  setStoreAddress,
}) => {
  const [enteredAddress, setEnteredAddress] = useState<string>('');
  const [searchList, setSearchList] = useState<DocumentAddress[]>([]);
  const [isView, setIsView] = useState<boolean>(false);
  const [position, setPosition] = useState<Coordinate | null>(null);

  useKakaoLoader({
    appkey: KAKAO_MAP_TOKEN,
  });

  const searchAddress = async (e: FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    ...
    const addresses = resData.documents.map((document: any) => {
      const documentAddress: DocumentAddress = {
        id: document.x,
  ...
  const viewAddressOnMap = (enteredCoordinate: Coordinate, address: string) => {
    setIsView(true);
    setPosition(enteredCoordinate);
              ...
              {searchList.map((searchItem) => (
                <li key={searchItem.id} className='address-item'>
                  ...
                  <button
                    ...
                    onClick={viewAddressOnMap.bind(
                      null,
                      searchItem.coordinate,
                      searchItem.address,
                    )}
                  >
                    지도보기
